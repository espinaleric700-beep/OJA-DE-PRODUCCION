...
with tabs[1]:
    ...
    with st.form("form_crear_orden_completa"):
        ...
        if "tallas_seleccionadas_state" not in st.session_state:
            st.session_state["tallas_seleccionadas_state"] = []

        col_add_t1, col_add_t2 = st.columns([3, 1])
        with col_add_t1:
            talla_a_agregar = st.selectbox("Seleccionar Talla a Agregar", options=[t for t in tallas_disponibles if t not in st.session_state["tallas_seleccionadas_state"]], key="select_talla_individual")
        with col_add_t2:
            st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
            if st.button("➕ Agregar Talla"):
                if talla_a_agregar and talla_a_agregar not in st.session_state["tallas_seleccionadas_state"]:
                    st.session_state["tallas_seleccionadas_state"].append(talla_a_agregar)
                    st.rerun()

        tallas_seleccionadas = st.session_state["tallas_seleccionadas_state"]
        
        dict_detalle_tallas = []
        if tallas_seleccionadas:
            st.markdown("---")
            st.caption("Especifique la cantidad y detalles/comentarios para cada talla agregada:")
            
            tallas_a_eliminar = []
            for sz in tallas_seleccionadas:
                c_cant, c_obs, c_del = st.columns([1, 2.2, 0.8])
                with c_cant:
                    cant_sz = st.number_input(f"Cantidad {sz}", min_value=1, value=1, step=1, key=f"form_cant_sz_{sz}")
                with c_obs:
                    obs_sz = st.text_input(f"Comentario {sz}", placeholder="Ej: Nombre Juan #10...", key=f"form_obs_sz_{sz}")
                with c_del:
                    st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
                    if st.button("🗑️ Quitar", key=f"btn_quitar_talla_{sz}"):
                        tallas_a_eliminar.append(sz)
            ...
            if tallas_a_eliminar:
                for sz_del in tallas_a_eliminar:
                    st.session_state["tallas_seleccionadas_state"].remove(sz_del)
                st.rerun()
