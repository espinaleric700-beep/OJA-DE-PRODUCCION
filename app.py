...
ancho_pantalla = streamlit_js_eval(
    js_expressions="window.innerWidth", key="viewport_width"
)
...
def actualizar_estado_con_historial(
    o_id, estado_anterior, nuevo_estado, historial_actual, usuario_actual
):
    ...
    supabase.table("ordenes").update(
        {"estado": nuevo_estado, "historial": json.dumps(lista_historial)}
    ).eq("id", o_id).execute()
...
def obtener_badge_estado(estado):
    ...
    color_texto, color_bg = colores.get(
        estado, ("#8b949e", "rgba(139, 148, 158, 0.15)")
    )
    ...
    if not st.session_state["autenticado"]:
        ...
        with col_b:
            if st.button("Iniciar Sesión", key="btn_login_top", use_container_width=True):
                if (
                    usuario_input.strip().lower() == "admin"
                    and password_input == "2580Admin"
                ):
                    ...
                    try:
                        ...
                        usuario_encontrado = next(
                            (
                                u
                                for u in res.data
                                if u["usuario"].lower() == usuario_input.lower()
                            ...
                            st.session_state.update(
                                {
                                    ...
                                    "rol": usuario_encontrado.get("rol_id", ""),
                                }
                            )
                            st.rerun()
                        else:
...
with tabs[0]:
    ...
    with col_f1:
        busqueda = st.text_input(
            "🔍 Buscar Cliente o # Orden",
            placeholder="Ej: 0000001 o Juan Perez",
            key="busqueda_ordenes_input",
        )
    with col_f2:
        filtro_estado = st.selectbox(
            "Filtrar Estado", ["Todos"] + lista_estados, key="filtro_estado_ordenes_frag"
        )
    ...
    try:
        ...
        if busqueda:
            ...
            ordenes = [
                o
                for o in ordenes
                if termino in str(o.get("numero_orden", "")).lower()
                or termino in o.get("nombre_cliente", "").lower()
            ]
            ...
            for o in ordenes:
                ...
                with st.container(border=True):
                    ...
                    with col_res:
                        ...
                        st.markdown(
                            f"### Orden #{numero_o} - **{cliente_o}** {badge_html}",
                            unsafe_allow_html=True,
                        )
                    ...
                    with col_act:
                        ...
                        idx_actual = (
                            lista_estados.index(estado_actual)
                            if estado_actual in lista_estados
                            else 0
                        )
                        with cols_action[0]:
                    ...
                    with st.expander("📂 Ver detalles completos"):
                        ...
                        with col_info1:
                            if st.session_state["rol"] in [
                                "Administrador",
                                "Recepción",
                            ]:
                                st.write(
                                    f"**Teléfono:** {o.get('telefono', 'N/D')}"
                                )
                            st.write(
                                f"**Fecha Entrega:** {o.get('fecha_entrega', 'N/D')}"
                            )
                        ...
                        try:
                            lista_tallas = (
                                json.loads(tallas_db)
                                if isinstance(tallas_db, str)
                                else tallas_db
                            )
                            ...
                            if st.session_state["rol"] in [
                                ...
                                if st.button(
                                    "✏️ Editar Tallas y Cantidades",
                                    key=f"btn_toggle_edit_tallas_{o_id}",
                                ):
                                    st.session_state[edit_mode_key] = not st.session_state[
                                        edit_mode_key
                                    ]
                                    st.rerun()
                            ...
                            if st.session_state[edit_mode_key]:
                                ...
                                with st.form(key=f"form_edit_tallas_{o_id}"):
                                    ...
                                    tallas_a_editar = st.multiselect(
                                        ...
                                        default=list(tallas_existentes_map.keys()),
                                        key=f"ms_edit_{o_id}",
                                    )

                                    temp_tallas_actualizadas = []
                                    for sz in tallas_a_editar:
                                        datos_previos = tallas_existentes_map.get(
                                            sz, {"cantidad": 1, "comentario": ""}
                                        )
                                    ...
                                    if st.form_submit_button(
                                        "💾 Guardar Cambios de Tallas"
                                    ):
                                        supabase.table("ordenes").update(
                                            {
                                                "tallas_detalle": json.dumps(
                                                    temp_tallas_actualizadas
                                                )
                                            }
                                        ).eq("id", o_id).execute()
                                        st.session_state[edit_mode_key] = False
                                        st.success(
                                            "¡Tallas actualizadas correctamente!"
                                        )
                                        st.rerun()
                            else:
                            ...
                            st.caption(
                                f"No se pudo procesar la información de tallas: {err}"
                            )
                        ...
                        try:
                            lista_archivos = (
                                json.loads(archivos_db)
                                if isinstance(archivos_db, str)
                                else archivos_db
                            )
                            if lista_archivos:
                                for idx_arch, item_file in enumerate(lista_archivos):
                                    ...
                                    nom_f = (
                                        item_file.get(
                                            "nombre", f"Archivo {idx_arch+1}"
                                        )
                                        if isinstance(item_file, dict)
                                        else f"Archivo {idx_arch+1}"
                                    )
                                    if url_f:
                        ...
                        try:
                            registros = (
                                json.loads(historial_db)
                                if isinstance(historial_db, str)
                                else historial_db
                            )
                            if registros:
                                for reg in registros[:5]:
                                    st.caption(
                                        f"🕒 {reg.get('fecha', '-')} | 👤 {reg.get('usuario', '-')}: {reg.get('de', '')} ➡️ {reg.get('a', '')}"
                                    )
                        except:
                        ...
                        if st.session_state["rol"] == "Administrador":
                            ...
                            if not st.session_state.get(confirm_key, False):
                                if st.button(
                                    f"🗑️ Eliminar Orden #{numero_o}",
                                    key=f"btn_init_del_orden_{o_id}",
                                ):
                                    st.session_state[confirm_key] = True
                                    st.rerun()
                            else:
                                st.warning(
                                    f"⚠️ ¿Seguro que deseas eliminar permanentemente la Orden #{numero_o}?"
                                )
                                col_del_yes, col_del_no = st.columns(2)
                                with col_del_yes:
                                    if st.button(
                                        "✅ Sí, Eliminar",
                                        key=f"btn_confirm_del_yes_{o_id}",
                                    ):
                                        try:
                                            ...
                                            st.success(
                                                f"Orden #{numero_o} eliminada exitosamente."
                                            )
                                            st.rerun()
                                        except Exception as err:
                                            st.error(
                                                f"Error al eliminar la orden: {err}"
                                            )
                                with col_del_no:
                                    if st.button(
                                        "❌ Cancelar",
                                        key=f"btn_confirm_del_no_{o_id}",
                                    ):
                                        st.session_state[confirm_key] = False
                                        st.rerun()

        else:
            st.info("No hay órdenes encontradas.")
    except Exception as e:
        st.error(f"Error: {e}")
...
with tabs[1]:
    ...
    with st.form("form_crear_orden_completa"):
        ...
        with col_c1:
            st.text_input(
                "Número de Orden (Auto)", value=numero_auto, disabled=True
            )
            nombre_cliente = st.text_input("Nombre del Cliente")
            telefono_cliente = st.text_input("Teléfono")
        with col_c2:
            tipo_servicio = st.selectbox(
                "Tipo de Servicio", ["Bordado", "DTF", "Sublimación", "Mixto"]
            )
        ...
        with col_m2:
            abono_orden = st.number_input(
                "ABONO / ANTICIPO ($)", min_value=0.0, step=100.0
            )
        ...
        if tallas_seleccionadas:
            st.caption(
                "Especifique la cantidad y detalles/comentarios para cada talla seleccionada:"
            )
            for sz in tallas_seleccionadas:
                c_cant, c_obs = st.columns([1, 2.5])
                with c_cant:
                    cant_sz = st.number_input(
                        f"Cantidad Size {sz}",
                        min_value=1,
                        value=1,
                        step=1,
                        key=f"form_cant_sz_{sz}",
                    )
                with c_obs:
                    obs_sz = st.text_input(
                        f"Comentario/Detalles Size {sz}",
                        placeholder="Ej: Nombre Juan #10, manga corta...",
                        key=f"form_obs_sz_{sz}",
                    )
            ...
            if archivos_subidos:
                with st.spinner("Subiendo archivos..."):
                    for arch in archivos_subidos:
                        url_file = subir_a_supabase(
                            arch.getvalue(),
                            arch.name,
                            bucket="disenos",
                            carpeta="ordenes_archivos",
                        )
...
with tabs[3]:
    ...
    if st.session_state["rol"] == "Administrador":
        with st.form("form_crear_usuario"):
            ...
            nuevo_rol = st.selectbox("Rol del Usuario", roles_disponibles)
            if st.form_submit_button("💾 Guardar Usuario"):
                if nuevo_user.strip() and nuevo_pass.strip():
                    try:
                        ...
                        st.error(f"Error al crear usuario: {err}")
                else:
