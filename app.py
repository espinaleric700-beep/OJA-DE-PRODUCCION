...
# Actualización automática cada 10 segundos (10,000 ms)
...
st.markdown("""
    ...
    div[data-testid="stForm"] { background-color: #111827; border: 1px solid #374151; padding: 15px; border-radius: 10px; }
...
# --- TAB 0: ÓRDENES ---
with tabs[0]:
    st.subheader("Control de Producción (Actualización cada 10s)")
    ...
    if ordenes:
        for o in ordenes:
            ...
            with c1:
                with st.expander(f"ORD: {o.get('numero_orden')} | Cliente: {o.get('nombre_cliente')} | Estado: {o.get('estado')}"):
                    st.write(f"**Piezas:** {o.get('piezas')} | **Área:** {o.get('area_produccion')}")
                    st.write(f"**Detalles:** {o.get('nombre_orden')}")
                    st.markdown(f"🕒 **Historial:** {o.get('historial')}")
            with c2:
                with st.form(f"f_{o.get('id')}"):
                    ...
                    nuevo = st.selectbox("Estado", estados, index=idx, label_visibility="collapsed")
                    if st.form_submit_button("Actualizar"):
                        hist = f"Cambio a {nuevo} el {datetime.now().strftime('%d/%m %H:%M')}"
...
with tabs[1]:
    with st.form("n_orden", clear_on_submit=True):
        ...
        detalles = col2.text_area("Detalles Técnicos")
        ...
        if st.form_submit_button("Crear Orden"):
            supabase.table("ordenes").insert({
                ...
                "estado": "Pendiente",
                "historial": f"Creado {datetime.now().strftime('%d/%m %H:%M')}"
            }).execute()
